...
public:
  ...
  int getContent(int xAxis, int yAxis) const;
  ...
  // - param 1: an int for X (column)row
  // - param 2: an int for Y (row)
  // - param 3: an int representing the content we want to set at this location
  void setContent(int xAxis, int yAxis, int content);
  ...
  void setContent(const std::vector<Point>& pts, int content);
  ...
  bool areAllLocsEmpty(const std::vector<Point>& pts) const;
...
private:  // This is commented out to allow us to test.
  ...
  static bool isValidPoint(int xAxis, int yAxis);
  ...
  const bool isRowCompleted(int rowIndex) const;
  ...
  const std::vector<int> getCompletedRowIndices() const;
